@php
    ...
    // Sensible starting values on a fresh Add - less typing for the
    // accountant, still fully editable. Only applied when actually empty so
    // this never overwrites a real value on Edit or a resubmitted form.
    if ($add) {
        if (empty($dataTypeContent->transaction_date)) {
            $dataTypeContent->transaction_date = now()->format('Y-m-d');
        }
        if (empty($dataTypeContent->conversion_rate)) {
            $dataTypeContent->conversion_rate = setting('payments.max_conversion_rate');
        }
    }
@endphp
